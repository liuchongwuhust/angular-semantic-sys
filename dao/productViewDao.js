...
sql.getProductDetails = function(req,res) {
		var querySql = 'select * from product where id ="'+req.session.productID+'"';
...
sql.getProductAmount = function(req,res) {
	var querySql="select count(id) as productAmount  from product";
	...
	sql.getFutures = function(req,res) {
		var querySql="select * from futures_position where proid="+req.session.productID+" and protype=1 order by date desc";
	...
	sql.getProductsWithLatestNet = function(req,res) {
			var querySql = 'SELECT a.*,b.* FROM `product` a LEFT JOIN ( SELECT a.* FROM (SELECT * FROM `netproduct` ORDER BY DATE DESC) a GROUP BY proid ) b ON a.`id` =b.proid ORDER BY  b.yearrate DESC';
			console.log("querySql",querySql);
    ...
    querySql+=     'LEFT JOIN product  b ON a.proid =b.id ';
    querySql+=      'LEFT JOIN  ( SELECT proid,`date`,net,totalnet,accrate, yearrate ';
    querySql+=                     'FROM ( ';
    querySql+=                           'SELECT proid,`date`,net,totalnet,accrate, yearrate FROM  netproduct c ORDER BY `date` DESC ';
    querySql+=                           ') c ';
	...
	sql.getProductsByFilterWithLatestNet = function(req,res) {
			var querySql = 'SELECT a.*,b.* FROM `product` a ';
			querySql+='LEFT JOIN ( SELECT a.* FROM (SELECT * FROM `netproduct` ORDER BY DATE DESC) a GROUP BY proid ) b ON a.`id` =b.proid ';
	...
	sql.getProductLatestNet = function(req,res) {
			var querySql = 'select * from netproduct where proid="'+req.query.productID+'" and date=( select max(date) from netproduct where proid="'+req.query.productID+'")';
			console.log("querySql",querySql);
	...
	sql.getProductSpot=function(req, res){
		var querySql = 'select ID,date,DATE_FORMAT(date,"%Y-%m-%d") as mdate,code,cname,volume,volamt,last from spot where proid='+req.session.productID+' and protype='+req.query.protype+' order by date desc';
		console.log("querySql",querySql);
	...
	sql.getNetProduct=function(req, res){
		var querySql = 'select  ID,date,DATE_FORMAT(date,"%Y-%m-%d") as datetime,net,totalnet,accrate,yearrate from netproduct where proid="'+req.query.productID+'" order by date desc';
		console.log("querySql",querySql);
	...
	sql.getNetAssetValue=function(req, res){
		// var querySql = 'select net,date from netproduct where proid="'+req.query.productID+'" and date=DATE_FORMAT("'+req.query.netdate+'","%Y%m%d")';
		var querySql = 'select * from netproduct where proid="'+req.query.productID+'" and date="'+req.query.netdate+'"';
		console.log("querySql",querySql);
